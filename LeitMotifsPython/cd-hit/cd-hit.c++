...
int main(int argc, char **argv) {
  ...
  for (i=1; i<argc; i++) {
    ...
    else if (strcmp(argv[i], "-F" ) == 0) Frag_size = atoi(argv[++i]);
  ...
  int len_upper_bound;
  int len_cov;
  int frg1, frg2, segfb, flag;
  int aan_list_backup[MAX_SEQ];
  if (Frag_size) 
    for (NR_frag_no=0, i=0; i<NR_no; i++) NR_frag_no += (NR_len[i] - NAA) / Frag_size + 1;
  else 
    NR_frag_no = NR_no;
  ...
  for (sg_i=0; sg_i<SEG_no; sg_i++) {
    ...
    for (sg_j=sg_i-1; sg_j>=0; sg_j--) {
      ...
      // if it > len_upper_bound,
      // seqs in sg_i won't be compared to seqs in sg_j
      if ( NR_len[ NR_idx[SEG_e[sg_j]] ] > len_upper_bound ) continue;
      ...
      for (i1=SEG_b[sg_i]; i1<=SEG_e[sg_i]; i1++) {
        ...
        if (Frag_size) {
          ...
          // frg2 can be within two continus frg fromseed sequence
          frg2 = (len - NAA + option_b ) / Frag_size + 1 + 1;
          flag = check_this_short(len, seqi, has_aa2,
             ...
             NR_flag[i], NR_flag, len_upper_bound, check_this_info, check_this_infod);
        }
        else {
          flag = check_this(len, seqi, has_aa2,
