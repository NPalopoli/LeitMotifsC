...
// information
char cd_hit_ver[]  = "\t\t\t====== CD-HIT version 3.1.2 ======";
...
int diag_test_aapn_est_circular(int NAA1, char iseq2[], int len1, int len2, int *taap,
        INTs *aap_begin, INTs *aap_list, int &best_sum,
        int band_width, int &band_left, int &band_right, int required_aa1) {
  int i, i1, j, k, l, m, n;
  int *pp;
  ...
  if (nall > MAX_DIAG) bomb_error("in diag_test_aapn_est_circular, MAX_DIAG reached");
  for (pp=diag_score, i=nall; i; i--, pp++) *pp=0;
  ...
  for (i=0; i<len22; i++,i1++) {
    ...
    ch3 = iseq2[(i+2)%len2];
    ch4 = iseq2[(i+3)%len2];
    if ((ch1==4) || (ch2==4) || (ch3==4) || (ch4==4)) continue; //skip N
    c22 = ch1*NAA3 + ch2*NAA2 + ch3*NAA1 + ch4;
...
// END diag_test_aapn_est_circular
...
void IDX_TBL::init(int naa, int naan){
  ...
  if (is_aa) {
    ...
    else if ( NAA == 9 ) { mem_size = 10; }
    else if ( NAA ==10 ) { mem_size = 6; }
    else if ( NAA ==11 ) { mem_size = 3; }
    else if ( NAA ==12 ) { mem_size = 2; }
    else bomb_error("Something wrong!");
  }
...
// give a bigger capacity at initial or after clean
// if I know word_tbl will grow a lot
// so that I don't have to resize word_tbl so often
void IDX_TBL::init_capacity(int new_capacity) {
  int i, j, k;
  for (i=0; i<NAAN; i++) {
    if (capacity[i] >= new_capacity) continue;
    if (size[i]     >= new_capacity) continue;
    // now both size[i] and capacity[i] < new_capacity

    if (size[i]) { // backup, resize, restore
      if ( capacity[i] > buffer_size ) {
        delete [] buffer;
        buffer_size = capacity[i];
        if ((buffer = new int[buffer_size]) == NULL) bomb_error("Memory");
      }

      for (k=0; k<size[i]; k++) buffer[k] = seq_idx[i][k];
      if ( capacity[i] >0 ) delete [] seq_idx[i];
      if ((seq_idx[i] = new int[new_capacity]) == NULL) bomb_error("Memory");
      for (k=0; k<size[i]; k++) seq_idx[i][k] = buffer[k];

      for (k=0; k<size[i]; k++) buffer[k] = word_no[i][k];
      if ( capacity[i] >0 ) delete [] word_no[i];
      if ((word_no[i] = new INTs[new_capacity]) == NULL) bomb_error("Memory");
      for (k=0; k<size[i]; k++) word_no[i][k] = buffer[k];
    }
    else if (capacity[i]) { // resize
      delete [] seq_idx[i];
      delete [] word_no[i];
      if ((seq_idx[i] = new int[new_capacity]) == NULL) bomb_error("Memory");
      if ((word_no[i] = new INTs[new_capacity]) == NULL) bomb_error("Memory");
    ...
    capacity[i] = new_capacity;
  }
} // END IDX_TBL::init_capacity
...
// copied from above with only diff if j < 0 ...
// but every time resize array, make capacity 50% bigger
// instead of capacity += memsize
// in order to reduce times of resizing
// but cost more RAM
int IDX_TBL::add_word_list2_costly(int aan_no, int *aan_list, 
                           INTs *aan_list_no, int idx) {
  int i, j, k, i1, j1, k1, i0, j0, k0;
  int mem_inc;
  ...
  for (j0=0; j0<aan_no; j0++) {
    if ( j1=aan_list_no[j0] ) {
      ...
      if ( size[j] == capacity[j] ) { // resize array
        ...
        mem_inc = (capacity[j]/2 > mem_size) ? capacity[j]/2 : mem_size;

        for (k=0; k<size[j]; k++) buffer[k] = seq_idx[j][k];
        if ( capacity[j] >0 ) delete [] seq_idx[j];
        if ((seq_idx[j] = new int[mem_inc+capacity[j]]) == NULL)
          bomb_error("Memory");
        for (k=0; k<size[j]; k++) seq_idx[j][k] = buffer[k];

        for (k=0; k<size[j]; k++) buffer[k] = word_no[j][k];
        if ( capacity[j] >0 ) delete [] word_no[j];
        if ((word_no[j] = new INTs[mem_inc+capacity[j]]) == NULL)
          bomb_error("Memory");
        for (k=0; k<size[j]; k++) word_no[j][k] = buffer[k];
  ...
  return OK_FUNC;
} // END int IDX_TBL::add_word_list2_costly


int IDX_TBL::print_all() {
  int  i, j, k, j0, j1, k1;
  int cols = 0;
  ...
  for (i=0; i<NAAN; i++) {
    if ( size[i] == 0 ) continue;
    cols++;
    cout << k << "\t" << i << "\tsize:" << size[i] << "\t";
    for (j=0; j<size[i]; j++) {
      cout << seq_idx[i][j] << "," << word_no[i][j] << " ";
      total_words += word_no[i][j];
    }
    cout << endl;
    k++;
  }
...
int IDX_TBL::pop_long_seqs(int upper_bound) {
  ...
  if (upper_bound + 1 - libb_last > lib_step) {
    libb_last = upper_bound;
  }
  else {
    return OK_FUNC;
  }
...
int db_read_in (ifstream &in1, char *db_bin_swap, 
  ...
  while(1) {
    ...
    if (in1.fail() && (buffer1[0] == '>') )
      bomb_error("file reading error, possible a description line is too long");
    if ( buffer1[0] == '>') {
      if ( read_in ) { // write previous record
         format_seq(raw_seq);

         if ( strlen(raw_seq) > option_l ) {
  ...
  in1.close();
  if (option_B) bindb[bin_no].close();
...
// modified from above, but skip option_l and format_seq
...
int sort_seqs_divide_segs (int option_B,
  ...
  for (i=0; i<NR_no; i++) {
    ...
    if (! option_B) setiseq(NR_seq[i], len);
  }
  if (max_len >= 65536) 
  ...
  if (option_B) mem_limit -=                29*NR_no + 16 * NAAN;
  ...
  if (option_B) mem_limit /= sizeof (int) + sizeof (INTs) + 2*sizeof(char);
...
int db2_seqs_divide_segs (int option_B,
...
int db_read_des(ifstream &in1, 
                int option_l, int des_len, char *NR_seq[]) {
...
int remove_tmp_files_db2(int option_B, char db_bin_swap[]) {
  char cmd[256];
  int i, j, k;
...
int upper_bound_length_rep(int len, double opt_s,  int opt_S,
...
//for cd-hit-2d
//got upper and lower bound of length need to considered
//because if aignment coverage parameters, sequences maybe too long or too short
//to be considered
int set_len_bound(int &len_upper_bound, int &len_lower_bound, int &len_upper_bound2, int &len_lower_bound2,
                int NR_no, int *NR_len, int NR2_no, int *NR2_len,
                double option_s,  int option_S,  double option_aL, int option_AL,
                double option_s2, int option_S2, double option_aS, int option_AS) {

  int i, j, k, ll, len, len_tmp;
  int max_len1 = 0, min_len1 = 99999999;
  int max_len2 = 0, min_len2 = 99999999;
  int db_out_of_bound = 0;
  for (i=0; i<NR_no;  i++) { ll = NR_len[i];  if (ll>max_len1) max_len1=ll; if (ll< min_len1) min_len1=ll;}
  for (i=0; i<NR2_no; i++) { ll = NR2_len[i]; if (ll>max_len2) max_len2=ll; if (ll< min_len2) min_len2=ll;}

  //set upper bound of seqs in db1
  len_upper_bound = upper_bound_length_rep(max_len2, option_s, option_S, option_aL, option_AL);
  //if (len_upper_bound > max_len1) len_upper_bound = max_len1;

  //set lower bound of seqs in db1
  len_lower_bound = min_len2 - option_S2;
  len_tmp = (int) ( ((double)min_len2) * option_s2);
  if (len_tmp < len_lower_bound) len_lower_bound = len_tmp;
  //if (len_lower_bound < min_len1) len_lower_bound = min_len1;

  //set upper bound of seqs in db2
  len_upper_bound2 = upper_bound_length_rep(max_len1, option_s2, option_S2, option_aS, option_AS);
  //if (len_upper_bound2 > max_len2) len_upper_bound2 = max_len2;

  //set lower bound of seqs in db2
  len_lower_bound2 = min_len2;
  double r1 = (option_s < option_aL) ? option_s : option_aL;
  int    a2 = (option_S > option_AL) ? option_S : option_AL;
  if (r1 > 0.0) len_lower_bound2 = (int) (((float) min_len1) * r1);
  if ((min_len1-a2) < len_lower_bound2) len_lower_bound2 = min_len1-a2;
  //if (len_lower_bound2 < min_len2) len_lower_bound2 = min_len2;

  if (min_len1 > len_upper_bound)  db_out_of_bound=1;
  if (max_len1 < len_lower_bound)  db_out_of_bound=1;
  if (min_len2 > len_upper_bound2) db_out_of_bound=1;
  if (max_len2 < len_lower_bound2) db_out_of_bound=1;

  return db_out_of_bound;
}
// END set_len_bound
...
// when alignment coverage such as -aL is specified
// if a existing rep is too long, it won't be qulified 
// since existing reps are sorted in length
// given len_upper_bound, the existing reps that longer than it don't need to be compared
int libb_curr_test(int len_upper_bound, int libb, int libe, int *NR_len, int *NR90_idx) {
  int i, j, k, len2;

  for (i=libb; i<=libe; i++) {
    len2 = NR_len[NR90_idx[i]];
    if (len2 <=len_upper_bound ) { return i;}
  }
  return libb;
} // END libb_curr_test
...
void calc_required_aaxN_circular(int &required_aa1, int &required_aas, int &required_aan,
                       double NR_clstr, int len_eff, int NAA, int ss,
                       int *word_stat, int *word_ss_stat, int tolerance) {
  int len_bin;
  if (tolerance) {
    required_aa1 = int (NR_clstr * (double) len_eff);
    len_bin = len_eff/100;
    if (len_bin>4) len_bin=4;
    required_aan = word_stat[len_bin];
    required_aas = word_ss_stat[len_bin];
  }
  else {
    required_aa1 = int (NR_clstr * (double) len_eff);
    required_aan = len_eff-(len_eff-required_aa1)*NAA;
    required_aas = len_eff-(len_eff-required_aa1)*ss;
  }
} // END calc_required_aaxN_circular


void make_comp_short_word_index(int NAA, int *NAAN_array, int *Comp_AAN_idx) {
  int i, j, k, icomp, k1;
  int c[4] = {3,2,1,0};
  char short_word[32]; //short_word[12] is enough
